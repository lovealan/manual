...
<script>
    function restate(id, state) {
        $.ajax({
            type: "POST",
            url: "{:$router->buildUrl('/xielei/manual/manual/update')}",
            data: {
                id: id,
                state: state,
            },
    ...
    function deleteItem(id, title) {
        if (confirm("删除“" + title + "”\r\n此操作无法恢复，确定删除？")) {
            $.ajax({
                type: "POST",
                url: "{:$router->buildUrl('/xielei/manual/manual/delete')}",
                data: {
                    id: id,
                },
                dataType: "JSON",
        ...
        <tbody>
            ...
            <tr>
                <td class="text-nowrap">
                    ...
                    <a href="{:$router->buildUrl('/xielei/manual/post/index', ['manual_id'=>$vo['id']])}">{$vo.title}</a>
                </td>
                <td>
                    {if $vo['state']==1}
                ...
                <td class="text-nowrap">
                    <a href="{:$router->buildUrl('/xielei/manual/manual/update', ['id'=>$vo['id']])}">设置</a>
...
<a href="{:$router->buildUrl('/xielei/manual/manual/create')}" class="btn btn-primary">创建手册</a>

...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{:$router->buildUrl('/xielei/manual/manual/index')}">手册管理</a></li>
        <li class="breadcrumb-item"><a href="{:$router->buildUrl('/xielei/manual/post/index', ['manual_id'=>$manual['id']])}">{$manual['title']}</a></li>
        {foreach $pdata as $vo}
        <li class="breadcrumb-item"><a href="{:$router->buildUrl('/xielei/manual/post/index', ['manual_id'=>$manual['id'],'pid'=>$vo['id']])}">{$vo.title}</a></li>
        {/foreach}
...
<script>
    function create(type) {
        ...
        if (title = prompt(tips)) {
            $.ajax({
                type: "POST",
                url: "{:$router->buildUrl('/xielei/manual/post/create')}",
                data: {
    ...
    function rename(id, title) {
        if (title = prompt("请输入新的名称", title)) {
            $.ajax({
                type: "POST",
                url: "{:$router->buildUrl('/xielei/manual/post/update')}",
                data: {
                    id: id,
                    title: title,
                },
    ...
    function deleteItem(id, title) {
        if (confirm("删除“" + title + "”\r\n此操作无法恢复，确定删除？")) {
            $.ajax({
                type: "POST",
                url: "{:$router->buildUrl('/xielei/manual/post/delete')}",
                data: {
                    id: id,
                },
                dataType: "JSON",
    ...
    function rerank(id, type) {
        $.ajax({
            type: "POST",
            url: "{:$router->buildUrl('/xielei/manual/post/rank')}",
            data: {
                id: id,
                type: type,
            },
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="text-nowrap">
                    ...
                    <a href="{:$router->buildUrl('/xielei/manual/post/index', ['pid'=>$vo['id'],'manual_id'=>$vo['manual_id']])}">{$vo.title}</a>
                    {elseif $vo['type']==2}
                    ...
                    <a href="{:$router->buildUrl('/xielei/manual/post/update', ['id'=>$vo['id']])}">{$vo.title}</a>
                    {/if}
                </td>
                <td>
                    {if $vo['state']==1}
    ...
    $(document).ready(function() {
        ...
        $("#inlineFormCustomSelect").bind('change', function() {
            ...
            if (pid >= 0) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{:$router->buildUrl('/xielei/manual/post/move')}",
                    data: {
                        ids: ids,
                        pid: pid,
                    },

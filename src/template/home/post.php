...
<ul class="pl-3" style="list-style:none;">
    ...
    <li style="list-style:none;">
        ...
        <a href="{:$router->buildUrl('/xielei/manual/home/post',['id'=>$vo['alias']?:$vo['id']])}" class="{if $vo['id']==$cur_id}text-danger nav_current font-weight-bold{/if}">{$vo.title}</a>
        {/if}
    </li>
    {/foreach}
    {else}
...
<div class="row">
    <div class="col-md-3">
        ...
        <div class="bg-light mb-2 p-2">
            ...
            <a href="{:$router->buildUrl('/xielei/manual/home/manual',['id'=>$manual['alias']?:$manual['id']])}">{$manual.title}</a>
        </div>
        {:mulu($postModel, $manual['id'], 0, $post['id'], $router)}
    </div>
    <div class="col-md-9">
        {$post['body']}
    </div>
